...
public record UserAccess(Set<Roles> roles, Set<Modules> modules, Set<Submodules> submodules) {
	...
	private static void validateEverySubmoduleBelongsToAnAssignedModule(Set<Modules> modules,
			Set<Submodules> submodules) {
		Set<Long> assignedModuleIds = modules.stream().map(Modules::id).filter(Objects::nonNull)
				.collect(java.util.stream.Collectors.toSet());

		for (Submodules submodule : submodules) {
			Modules parentModule = submodule.module();
			if (parentModule == null || parentModule.id() == null) {
				throw new BusinessException("Every submodule must belong to a persisted module");
			}
			if (!assignedModuleIds.contains(parentModule.id())) {
				throw new BusinessException(
						"Submodule '" + submodule.name() + "' requires its parent module to be assigned");
			}
		}
	}
}
